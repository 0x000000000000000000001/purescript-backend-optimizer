...
isTypeClassDictionaryWithProps = go false
  ...
  go isClass = case _ of
    ...
    ExprTypeApp _ expr' _ -> go isClass expr'
  ...
  unrollApps = loop []
    where
    loop acc (ExprApp _ f x) = loop (Array.cons x acc) f
    loop acc (ExprTypeApp _ f _) = loop acc f
    loop acc e = Tuple e acc
